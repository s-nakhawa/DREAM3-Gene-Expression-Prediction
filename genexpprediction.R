...
# Make a dataset for the time specified. Columns = geneID, gene, wt, gat1, 
# gcn4, and leu3
df <- expdata[ , c(1, 2, tmcol)]
names(df) <- c("ProbeID", "geneName", "wt", "gat1", "gcn4", "leu3")

# Divide dataset into train and test data. These datasets will contain numerical
# expression values for each gene that has been observed. 
...
# Use kmeans clustering to assign cluster numbers to each expression level for
# each gene in each sample
# use NbClust package to determine the optimal number of clusters for expression
# levels in each sample
library(NbClust)
if (tm == 0) {
  set.seed(seed)
  kmwt <- kmeans(as.matrix(t10$wt), centers = 1) # the wild type expression data
  # at time = 0 seconds all have gene expression values of 0, so they cannot 
  ...
  nbwt <- NbClust(as.matrix(t10$wt), 
                  ...
                  max.nc = 10, 
                  method = "complete", 
                  index = "ch") # this function determines the optimal number 
                                # of clusters for the gene expression data of one 
                                # sample (in this case, it's the wild type sample)
  
  nclustwt <- nbwt$Best.nc[1] # the optimal number of clusters is assigned to 
                              # this variable, then used in the kmeans calculation 
                              # for this sample
  set.seed(seed)
  kmwt <- kmeans(as.matrix(t10$wt), centers = nclustwt)
}

# Each expression value is replaced with an assignment to a cluster from the
# kmeans algorithm
clusterwt <- kmwt$cluster

# This process is repeated for the other two samples that are not being 
# predicted (gcn4 and leu3 deletion).

# None of the other samples have all 0's as their expression values, so a 
# conditional (like the one used for the wild type data) is not necessary.
...
# A dataframe is created that contains the Probe IDs, Gene names, cluster 
# numbers for the predictor variables, and numerical expression values for the
# gat1 sample that is to be predicted.
...
# The dataset containing the cluster numbers as factors is divided into a testing
...
# The next step is to create a random forest using the training dataset and create
# a regression model that can predict expression values for various genes in the
# gat1 deletion sample based on the expression values of the same genes in the 
# wild type, gcn4, and leu3 samples.
...
# In order to maximize the effectiveness of the random forest, a tuning grid 
# search will be performed to determine the optimal parameters of the random forest.

# The following tuning grid tests the optimal values for the number of trees, 
# variables to consider at each split, the target size of the node, and the 
# optimal sample size. The best combination of parameters will be the one with
...
# The tuning grid is tested by creating random forests for each combination \
# of parameters. 
...
# The resulting tuning grid containing the RMSE values for each combination of 
# parameters is sorted from least to greatest. The combination of parameters that
# minimized the RMSE is the combination that will be used to create the random 
# forest for the predictive regression model. 
rfgrid <- rfgrid %>%
  arrange(RMSE)
...
# Make predictions for the expression levels of the genes in the gat1 deletion 
# sample.
...
# Arrange the final dataset that contains the gene expression predictions to be
# in alphabetical order by gene_name, so that it may be compared to the Gold 
# Standard rankings. 
...
# dataset, so the ranks from the Golden Standard will be adjusted.

# The absent genes are BAT1 (#7), DSE2 (#12), SPL2 (#39)
nogene <- c(7, 12, 39)
gs <- goldstandard [-nogene, c(1, 2, goldcol)] #this creates a dataframe that 
                                               # deletes the rows of absent genes 
                                               # and restricts the columns to the 
                                               # Probe IDs, gene names, and the specific 
                                               # time we are looking at. 

names(gs) <- c("probeID", "gene_name", "ranktime") # the third column, now named "ranktime", 
                                                   # has been converted into a generic name so
                                                   # that this column can be manipulated no 
                                                   # matter what time these rankings are for

gs <- arrange(gs, ranktime) # the genes are arranged by their rank

gs$ranktime <- 1:nrow(gs) # these ranks are reordered to reflect the changes that
                          # have been made by deleteing three genes. 

gs <- arrange(gs, gene_name) # the genes are then arranged by alphabetical order
                             # again so that they can be compared

# Predicted ranks of gene expression for this particular time in the gat1 deletion 
...
# This is repeated 8 times: one for each time the gene expressions in the 
...
# for the gat1 deletion sample at each time it was tested.


# Create a new Gold Standard dataframe that modifies the rankings. This is 
# because three genes were absent in the testing dataset.
...
pvalcol <- numeric(length = 8) # create an empty vector where the p-values will be
